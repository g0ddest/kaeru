...
## 12a. Дорожная карта после плана 3 (зафиксировано 2026-09-13)

**План 4 — совместный просмотр.** Несколько человек смотрят одну серию синхронно и общаются в приложении.
- Синхронизация: комната с ведущим; ведущий рассылает состояние (аниме, серия, озвучка, позиция, play/pause, seek) через WebRTC data channel; ведомые подстраивают свой плеер с компенсацией задержки. Каждый участник резолвит СВОЙ поток Kodik (ссылки привязаны к IP, делиться ими нельзя) — синхронизируется только состояние воспроизведения, не видео.
- Соединение: WebRTC P2P (STUN публичный; для NAT-пробивания понадобится TURN). Для сигналинга без своего бэкенда — обмен приглашением через ссылку/QR (`kaeru://room?...`) недостаточен из-за динамических ICE-кандидатов, поэтому потребуется минимальный сигнальный сервис (Firebase Realtime/Supabase Realtime или крошечный WebSocket-сервер). Это первое место, где у приложения появляется бэкенд-зависимость; решение принимается при планировании плана 4.
- Чат: текстовый чат в комнате поверх data channel (без сервера истории; история живёт пока комната открыта), реакции. Голос: аудиотрек WebRTC как опция «Голосовой чат» с push-to-talk/мутом; экономия ресурсов на ТВ.
- ТВ: телевизор — ведомый участник комнаты; чат читается поверх видео, вводится с телефона (телефон как пульт комнаты).
- Ограничения: 2–6 участников, аккаунты Shikimori как идентичность (ник, аватар), без модерации.

**План 5 (кандидаты):** уведомления о новых сериях с картинкой и действием «Смотреть», виджет «Продолжить», история просмотра, экспорт/импорт настроек.

### Chromecast: брендированный приёмник и Cast Connect (добавлено 2026-09-13)
- Сделано в плане 3 (задача 5c): Styled Media Receiver `0EEA38FE` (опубликован), скин `docs/cast/` через GitHub Pages — логотип, сплэш, акцентный прогресс, водяной знак.
- Кандидат плана 5: Custom Receiver на CAF (своя HTML-страница на том же хостинге) — экран ожидания в стиле Kaeru с названием и номером серии, карточка следующей серии и отсчёт на телевизоре, канал сообщений для совместного просмотра (план 4).
- Кандидат плана 5: Cast Connect (Android TV Receiver) — каст с телефона на Android TV открывает само приложение Kaeru для ТВ вместо веб-приёмника; требует `CastReceiverContext` в ТВ-сборке и отметки Cast Connect в консоли.

### Офлайн-просмотр (добавлено 2026-09-13, запрос пользователя) — СДЕЛАНО в плане 4 (2026-09-15), спека `2026-09-15-kaeru-offline-design.md`
- Кандидат плана 5: загрузка серий на устройство и просмотр без сети. Техническая основа — media3 `DownloadManager`/`DownloadService` с поддержкой HLS (Kodik отдаёт HLS-плейлисты; ссылки подписаны на часы и привязаны к IP, поэтому загрузка стартует сразу после резолва и не возобновляется по старой ссылке — при обрыве резолв повторяется).
- UX: действие «Скачать» на серии в сетке тайтла и в плеере, управление скачанным — в настройках, раздел «Загрузки»: занятое место, список по тайтлам и сериям с размерами, удаление по одной серии и целиком, лимит места, качество загрузки, автоудаление после просмотра, «Только по Wi‑Fi», пометка скачанных серий в сетке и на карточках, автоматическое удаление после просмотра по желанию; на главной в офлайне показываются только скачанные тайтлы и «Продолжить» работает без сети; прогресс и отметки просмотра копятся локально и синхронизируются с Shikimori при появлении сети.
- Ограничения: только телефон (на ТВ загрузки не нужны), без DRM; хранение в app-specific storage; уведомление о ходе загрузки.

### Совместный просмотр — идея пользователя (записано 2026-09-15, план 5, сначала ресёрч)
- Оба телефона с Kaeru. В плеере кнопка «Смотреть вместе» → формируется ссылка (диплинк или обычная), хост шлёт её другу в любом чате; друг открывает, приложение перехватывает.
- В ссылке — прямой адрес устройства и данные для установления P2P-связи.
- Дальше приложения обмениваются метаинформацией (серия, озвучка, позиция), текстовыми сообщениями (висят некоторое время, затем скрываются), эмоциями (реакции) и голосовыми сообщениями (просто проигрываются).
- Порядок: полноценный технический и интерфейсный ресёрч → предложение решения пользователю → совместное проектирование → реализация.
