...
| Жизненный цикл | Сессия живёт, пока плеер не закрыт насовсем: переживает поворот, сворачивание и PiP; заканчивается при выходе из плеера или освобождении воспроизведения (v1: без фонового таймаута — ушедший в чат зритель не теряет комнату; обрыв сети даёт 30 с на переподключение по тому же roomId) | Ruling при финальном ревью 2026-09-16; Doze‑обрыв ловится окном переподключения |
