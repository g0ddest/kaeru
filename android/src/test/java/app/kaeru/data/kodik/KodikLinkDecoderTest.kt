...
class KodikLinkDecoderTest {
    ...
    private fun encodeForShift(plain: String, n: Int): String {
        val base64 = Base64.getEncoder().withoutPadding().encodeToString(plain.toByteArray())
    ...
    fun `decode returns https urls with manifest for all qualities from real ftor response`() {
        ...
        result.values.forEach { url ->
            assertTrue("expected $url to start with https://", url.startsWith("https://"))
            assertTrue("expected $url to contain manifest.m3u8", url.contains("manifest.m3u8"))
        }
    }
