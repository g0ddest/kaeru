...
автоматически (приоритет пользователя → больше всего серий → первая по
списку) и запоминается на тайтл, качество можно сменить без потери позиции.
Позиция серии сохраняется каждые 5 секунд, при паузе и при выходе; после
