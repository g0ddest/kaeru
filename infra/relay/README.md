...
Адрес релея лежит в APK, так что кто угодно со сборкой может открывать комнаты; `Origin` воркер не
проверяет. Угадать чужую комнату нельзя — 64 бита случайности, — а вот выбрать суточную квоту
бесплатного тарифа можно. Поэтому подключение к `/w/*` ограничено в коде: 30 апгрейдов на IP в
минуту (`RateDO`, своя корзина на каждый маршрут). Двум телефонам за одним роутером хватает с
запасом — переподключение по расписанию 1/2/4/8/16 с это шесть дозвонов за плохие полминуты, — а
скрипту, открывающему комнаты подряд, нет. Кадры внутри уже открытого соединения по частоте не
считаются; правило Cloudflare WAF на `/w/*` остаётся возможным вторым эшелоном, но заводится в
панели и в репозитории не живёт.
